...
### Companion overview — biological mechanism side

The companion overview [[overviews/transcriptional-heterogeneity-as-developmental-mechanism]] covers the same cell-level heterogeneity from a different angle. Where this overview traces the *methodological lineage* for extracting program structure (PCA → NMF → joint decomposition → foundation-model embedding) and treats heterogeneity as a signal to be cleaned up into stable axes, the companion overview asks how that heterogeneity *acts as a biological mechanism* — driving lineage potential, fate specification, and NDD vulnerability.

The two overviews are complementary perspectives on the same phenomenon (methodology vs. biology), not strict scope partitions, and the underlying papers overlap (CellRank, GRN inference, lineage tracing all appear in both). Both overviews can use a stochastic-differential-equation lens — $dX_t = \mu(X, t)\,dt + \sigma(X, t)\,dW_t$ — to decompose heterogeneity into deterministic drift (cell type × time × position context) and stochastic diffusion (transcriptional bursting, fate sampling). This decomposition is a mathematical idealization, useful for auditing which papers isolate which source, but not a claim about NPC biology being a literal Langevin process.
